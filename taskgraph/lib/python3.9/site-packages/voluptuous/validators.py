...
class Replace:
    ...
    def __repr__(self):
        return "Replace({!r}, {!r}, msg={!r})".format(
            self.pattern.pattern, self.substitution, self.msg
        )
...
class Range:
    ...
    def __init__(
        self, min=None, max=None, min_included=True, max_included=True, msg=None
    ):
        self.min = min
        self.max = max
    ...
    def __repr__(self):
        return (
            ...
            " max_included={!r}, msg={!r})".format(
                self.min, self.max, self.min_included, self.max_included, self.msg
            )
        )
...
class Number:
    ...
    def __call__(self, v):
        ...
        if (
            ...
            raise Invalid(
                self.msg
                or "Precision must be equal to {}, and Scale must be equal to {}".format(
    ...
    def __repr__(self):
        return f"Number(precision={self.precision}, scale={self.scale}, msg={self.msg})"
    ...
    def _get_precision_scale(self, number):
        ...
        return (
            len(decimal_num.as_tuple().digits),
            -(decimal_num.as_tuple().exponent),
            decimal_num,
        )
...
class SomeOf(_WithSubValidators):
    ...
    def __repr__(self):
        return "SomeOf(min_valid={}, validators=[{}], max_valid={}, msg={!r})".format(
            self.min_valid,
            ", ".join(repr(v) for v in self.validators),
            self.max_valid,
            self.msg,
