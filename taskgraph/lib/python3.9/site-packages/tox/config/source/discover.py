...
def _load_exact_source(config_file: Path) -> Source:
    ...
    exact_match = next(
        (s for s in SOURCE_TYPES if config_file.name == s.FILENAME), None
    )  # pragma: no cover
    for src_type in (
        (exact_match,) if exact_match is not None else SOURCE_TYPES
    ):  # pragma: no branch
        try:
...
def _create_default_source(root_dir: Path | None) -> Source:
    if root_dir is None:  # if set use that
        ...
        empty = root_dir
    logging.warning(
        f"No {' or '.join(i.FILENAME for i in SOURCE_TYPES)} found, assuming empty tox.ini at {empty}"
    )
