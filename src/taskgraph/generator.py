...
class TaskGraphGenerator:
    ...
    def _run(self):
        ...
        # Current parallel generation relies on multiprocessing; this causes
        # problems on Windows due to how new processes are created there, and
        # how doing so reinitializes global variables that are modified
        # earlier in graph generation, that doesn't get redone in the new
        # processes. Ideally this would be fixed, or we would take another
        # approach to parallel kind generation. In the meantime, it's not
