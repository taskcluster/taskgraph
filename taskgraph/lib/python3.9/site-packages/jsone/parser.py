...
class SyntaxError(TemplateError):
    ...
    def unexpected(cls, got, exp):
        exp = ", ".join(sorted(exp))
        return cls(f"Found: {got.value} token, expected one of: {exp}")
...
class Tokenizer:
    def __init__(self, ignore, patterns, tokens):
        ...
        token_patterns = [
            f"({self.patterns.get(t, re.escape(t))})" for t in self.tokens
        ]
        if self.ignore:
            token_patterns.append(f"(?:{self.ignore})")
    ...
    def generate_tokens(self, source):
        ...
        while True:
            ...
            if not mo:
                if remainder:
                    raise SyntaxError(
                        f"Unexpected input for '{source}' at '{remainder}'"
                    )
                break
            offset += mo.end()
