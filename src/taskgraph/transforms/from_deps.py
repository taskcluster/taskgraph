...
from taskgraph.transforms.run import FetchesStruct
...
class FromDepsChildStruct(Struct):
    # Optional fields
    ...
    # `fetches` entry.
    # Keys are task kind names, values are lists of FetchesStruct objects.
    fetches: Optional[Dict[str, List[FetchesStruct]]] = None


# Schema for from_deps transforms
class FromDepsStruct(Struct, forbid_unknown_fields=False):
    """Schema for from_deps transforms."""

    from_deps: FromDepsChildStruct


transforms = TransformSequence()
transforms.add_validate(FromDepsStruct)
