...
class BaseStruct(Struct):
    ...
    code_review: Optional[CodeReviewStruct] = None
...
class Parameters(ReadOnlyDict):
    ...
    def check(self):
        ...
        # This validation is minimal - it only checks for required fields in strict mode
        # to match the original voluptuous behavior which allowed extra fields
        try:
            # Convert underscore keys to kebab-case since BaseStruct uses rename="kebab"
            kebab_params = {k.replace("_", "-"): v for k, v in self.items()}

            if self.strict:
                # Strict mode: validate required fields are present
                base_fields = msgspec.structs.fields(BaseStruct)

                # Find required fields (those without defaults)
                required_fields = set()
                for f in base_fields:
                    # A field is required if it has no default value
                    if (
                        f.default == msgspec.UNSET
                        and f.default_factory == msgspec.UNSET
                    ):
