...
def infixExpectationError(operator, expected):
    return InterpreterError(
        f"infix: {operator} expects {expected} {operator} {expected}"
    )
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        ...
        if node.token.kind == "+":
            ...
            try:
                ...
                raise InterpreterError(f'object has no property "{right}"')
    ...
    def visit_ContextValue(self, node):
        try:
            ...
            raise InterpreterError(f"unknown context value {node.token.value}")
        return contextValue
