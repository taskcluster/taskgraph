...
def humanize_error(
    data, validation_error, max_sub_error_length=MAX_VALIDATION_ERROR_ITEM_LENGTH
):
    ...
    if isinstance(validation_error, MultipleInvalid):
        ...
        return f"{validation_error}. Got {offending_item_summary}"


def validate_with_humanized_errors(
    data, schema, max_sub_error_length=MAX_VALIDATION_ERROR_ITEM_LENGTH
):
    try:
        return schema(data)
