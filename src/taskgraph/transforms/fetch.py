...
from ..util.schema import Schema, Struct
...
class BaseFetchStruct(Struct, forbid_unknown_fields=False):
...
class FetchStruct(Struct):
    ...
    description: str
    # Fetch configuration - validated as BaseFetchStruct which ensures 'type' exists
    # Additional type-specific validation is done by the fetch_builder decorator
    fetch: BaseFetchStruct
...
transforms.add_validate(FetchStruct)
...
class GPGSignatureStruct(Struct):
...
class StaticUrlFetchStruct(Struct):
    ...
    gpg_signature: Optional[GPGSignatureStruct] = None
...
@fetch_builder("static-url", StaticUrlFetchStruct)
...
@fetch_builder("git", GitFetchStruct)
