...
3. Fetches the image into your local Docker repository.
...
* ``<image name>`` - Build the image defined under ``taskcluster/docker/<image name>``
  locally and use that for the task.
* ``<repository>`` - Load the image from the specified Docker repository.
