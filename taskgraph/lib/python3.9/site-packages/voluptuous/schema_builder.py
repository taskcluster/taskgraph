...
def raises(exc, msg=None, regex=None):
    try:
        ...
        if msg is not None:
            assert str(e) == msg, f"{str(e)!r} != {msg!r}"
        if regex is not None:
            assert re.search(regex, str(e)), f"{str(e)!r} does not match {regex!r}"
...
class Schema:
    ...
    def __repr__(self):
        return "<Schema({}, extra={}, required={}) object at 0x{:x}>".format(
            self.schema,
    ...
    def _compile_object(self, schema):
        ...
        def validate_object(path, data):
            if schema.cls is not UNDEFINED and not isinstance(data, schema.cls):
                raise er.ObjectInvalid(f"expected a {schema.cls!r}", path)
    ...
    def extend(self, schema, required=None, extra=None):
        ...
        assert (
            type(self.schema) == dict and type(schema) == dict
        ), "Both schemas must be dictionary-based"
...
class Msg:
    ...
    def __repr__(self):
        return f"Msg({self._schema}, {self.msg}, cls={self.cls})"
...
class Inclusive(Optional):
    ...
    def __init__(
        self, schema, group_of_inclusion, msg=None, description=None, default=UNDEFINED
    ):
        super().__init__(schema, msg=msg, default=default, description=description)
