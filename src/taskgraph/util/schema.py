...
class Struct(
    ...
    setting forbid_unknown_fields=False in their class definition:
        ...
        class MySchema(Struct, forbid_unknown_fields=False):
            foo: str
    """
...
class IndexSearchOptimizationStruct(Struct):
    """Search the index for the given index namespaces."""

    index_search: list[str]


class SkipUnlessChangedOptimizationStruct(Struct):
...
class OptimizationTypeStruct(Struct, forbid_unknown_fields=False):
...
OptimizationType = Union[None, OptimizationTypeStruct]


# Task reference types using msgspec (with Struct suffix for clarity)
class TaskReferenceStruct(Struct):
    """Reference to another task (msgspec version)."""

    task_reference: str


class ArtifactReferenceStruct(Struct):
    """Reference to a task artifact (msgspec version)."""

    artifact_reference: str


class TaskRefTypeStruct(Struct, forbid_unknown_fields=False):
...
# msgspec version for task references (renamed to avoid conflict)
taskref_or_string_msgspec = Union[str, TaskRefTypeStruct]
...
def validate_optimization(value):
    ...
    if isinstance(value, dict):
        if "index-search" in value:
            try:
                return msgspec.convert(value, IndexSearchOptimizationStruct)
            except msgspec.ValidationError:
                pass
        if "skip-unless-changed" in value:
            try:
                return msgspec.convert(value, SkipUnlessChangedOptimizationStruct)
            except msgspec.ValidationError:
                pass
...
def validate_task_ref(value):
    ...
    if isinstance(value, dict):
        if "task-reference" in value:
            try:
                return msgspec.convert(value, TaskReferenceStruct)
            except msgspec.ValidationError:
                pass
        if "artifact-reference" in value:
            try:
                return msgspec.convert(value, ArtifactReferenceStruct)
            except msgspec.ValidationError:
                pass
    # Simple validation for dict types
    if isinstance(value, dict):
...
# Voluptuous schemas - keeping original names for backward compatibility
