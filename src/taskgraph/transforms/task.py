...
class TaskDescriptionTreeherderStruct(Struct, rename=None):
...
class TaskDescriptionIndexStruct(Struct, rename="kebab"):
...
class TaskDescriptionWorkerStruct(Struct, rename=None, forbid_unknown_fields=False):
...
class TaskDescriptionStruct(Struct, forbid_unknown_fields=False):
    ...
    treeherder: Optional[Union[bool, TaskDescriptionTreeherderStruct]] = None
    # Information for indexing this build so its artifacts can be
    # discovered. If omitted, the build will not be indexed.
    index: Optional[TaskDescriptionIndexStruct] = None
    ...
    run_on_projects: optionally_keyed_by(
        "build-platform", list[str], use_msgspec=True
    ) = None  # type: ignore
...
def payload_builder(name, schema):
    ...
    """
    # Verify the schema has required fields using __annotations__
    # (more reliable than msgspec.structs.fields() across Python versions)
    if "implementation" not in schema.__annotations__:
...
class DockerWorkerCacheStruct(Struct, rename="kebab"):
...
class DockerImageInTreeStruct(Struct, rename="kebab"):
    """In-tree generated docker image."""

    in_tree: str


class DockerImageIndexedStruct(Struct):
...
class DockerImageTypeStruct(Struct, forbid_unknown_fields=False):
...
DockerImageType = Union[
    ...
    DockerImageTypeStruct,  # docker image configs
]


class DockerWorkerArtifactStruct(Struct, rename=None):
...
class DockerWorkerPayloadStruct(Struct):
    ...
    caches: Optional[list[DockerWorkerCacheStruct]] = None
    # artifacts to extract from the task image after completion
    artifacts: Optional[list[DockerWorkerArtifactStruct]] = None
    # environment variables
    env: dict[str, taskref_or_string_msgspec] = msgspec.field(default_factory=dict)
...
@payload_builder("docker-worker", DockerWorkerPayloadStruct)
...
class GenericWorkerArtifactStruct(Struct, rename=None):
...
class GenericWorkerMountContentStruct(Struct, rename="kebab"):
...
class GenericWorkerMountStruct(Struct, rename="kebab"):
    ...
    content: Optional[GenericWorkerMountContentStruct] = None
...
class GenericWorkerPayloadStruct(Struct):
    ...
    command: list[Union[str, list[taskref_or_string_msgspec]]]
    # the maximum time to run, in seconds
    max_run_time: int

    # Optional fields
    # artifacts to extract from the task image after completion
    artifacts: Optional[list[GenericWorkerArtifactStruct]] = None
    # Directories and/or files to be mounted
    mounts: Optional[list[GenericWorkerMountStruct]] = None
    # environment variables
    env: dict[str, taskref_or_string_msgspec] = msgspec.field(default_factory=dict)
    # the exit status code(s) that indicates the task should be retried
    retry_exit_status: Optional[list[int]] = None
...
@payload_builder("generic-worker", GenericWorkerPayloadStruct)
...
class BeetmoverReleasePropertiesStruct(Struct):
...
class BeetmoverUpstreamArtifactStruct(Struct, rename=None, omit_defaults=False):
    """Upstream artifact definition for beetmover."""

    # taskId of the task with the artifact
    taskId: taskref_or_string_msgspec  # Can be string or task-reference dict
...
class BeetmoverPayloadStruct(Struct):
    ...
    # the maximum time to run, in seconds
    max_run_time: int
    release_properties: BeetmoverReleasePropertiesStruct
    # list of artifact URLs for the artifacts that should be beetmoved
    upstream_artifacts: list[BeetmoverUpstreamArtifactStruct]
...
@payload_builder("beetmover", BeetmoverPayloadStruct)
...
class InvalidPayloadStruct(
    Struct, rename=None, omit_defaults=False, forbid_unknown_fields=False
):
...
class AlwaysOptimizedPayloadStruct(
    Struct, rename=None, omit_defaults=False, forbid_unknown_fields=False
):
...
class SucceedPayloadStruct(Struct, rename=None, omit_defaults=False):
...
@payload_builder("invalid", InvalidPayloadStruct)
def build_invalid_payload(config, task, task_def):
    task_def["payload"] = "invalid task - should never be created"


@payload_builder("always-optimized", AlwaysOptimizedPayloadStruct)
@payload_builder("succeed", SucceedPayloadStruct)
