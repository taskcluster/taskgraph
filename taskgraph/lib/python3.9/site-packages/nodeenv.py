...
def get_root_url(version):
    if parse_version(version) > parse_version("0.5.0"):
        return f"{src_base_url}/v{version}/"
    else:
        return src_base_url
...
def get_node_bin_url(version):
    ...
    if is_WIN or is_CYGWIN:
        postfix = "-win-{arch}.zip".format(**sysinfo)
    elif is_x86_64_musl():
        postfix = "-linux-x64-musl.tar.gz"
    else:
        postfix = "-{system}-{arch}.tar.gz".format(**sysinfo)
...
def install_node_wrapped(env_dir, src_dir, args):
    ...
    logger.info(
        f" * Install {src_type} node ({args.node}) ", extra=dict(continued=True)
    )
