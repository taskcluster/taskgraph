...
Deferring Substitution for the Task Context Transforms
''''''''''''''''''''''''''''''''''''''''''''''''''''''

In some cases where both the :mod:`~taskgraph.transforms.matrix` and
:mod:`~taskgraph.transforms.task_context` transforms are being used, you may
want to defer substitution entirely and let the ``task_context`` transforms
handle it. To do this, you can set ``defer-substitution: true``:
...
.. code-block:: yaml
   ...
   tasks:
     test:
       matrix:
         defer-substitution: true
         os: ["win"]

This will format the task definition to follow the ``task_context`` schema. If
passing ``defer-substitution: true``, the ``task_context`` transforms **must** run
afterwards, or else a task generation error will occur.
...
This will cause all combinations where ``os == mac`` to be skipped, as well as
the specific combination where ``os == win`` and ``python == py311``.
