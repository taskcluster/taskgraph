...
from .shared import JSONTemplateError, fromNow, string, to_str
            ...
            if variadic:
                ...
                def invoke(context, *args):
                    if minArgs:
                        if len(args) < minArgs:
                            raise BuiltinError(
                                f"invalid arguments to builtin: {name}: expected at least {minArgs} arguments"
                            )
                    for arg in args:
                        if not variadic(arg):
                            raise BuiltinError(f"invalid arguments to builtin: {name}")
