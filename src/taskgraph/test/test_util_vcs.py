...
def git_repo(tmpdir):
    ...
    subprocess.check_output(['git', 'commit', '-m', 'First commit'], cwd=repo_dir, env=env)
...
def _build_env_with_git_date_env_vars(date_time_string):
    env = os.environ.copy()
    env.update({
        env_var: date_time_string
        for env_var in _GIT_DATE_ENV_VARS
    })
    return env
...
@pytest.mark.parametrize('repo_type, commit_message', ((
    'hg', u'commit message in… pure utf8',
), (
    'hg', u'commit message in... ascii',
)))
def test_get_commit_message_hg(hg_repo, repo_type, commit_message):
    _test_get_commit_message(hg_repo, repo_type, commit_message, expected_result=commit_message)
