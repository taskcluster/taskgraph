...
MATRIX_SCHEMA = Schema(
    {
        ...
        Optional("matrix"): {
            Optional(
                ...
                description=dedent(
                    """
                Don't perform any task substitution (default: false).

                If specified, the ``task_context`` transforms **must** run afterwards.
                """.lstrip()
                ),
            ): bool,
            Optional(
...
def split_matrix(config, tasks):
    for task in tasks:
        ...
        defer = matrix.pop("defer-substitution", False)
        ...
        for new_task in new_tasks:
            ...
            if defer:
                task_context = new_task.setdefault("task-context", {})
                task_context["substitution-fields"] = list(fields)
                task_context["from-object"] = {
