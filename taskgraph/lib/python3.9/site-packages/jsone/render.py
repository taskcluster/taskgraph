...
from .shared import DeleteMarker, JSONTemplateError, TemplateError, string, to_str
    ...
    while True:
        ...
        if mo.group() != "$${":
            ...
            if isinstance(parsed, (list, dict)):
                raise TemplateError(
                    f"interpolation of '{string[:offset]}' produced an array or object"
                )
            if parsed is None:
                result.append("")
            else:
    ...
    if is_obj:
        ...
        for e in gen(value):
            if not isinstance(e, dict):
                raise TemplateError(
                    f"$map on objects expects {each_key} to evaluate to an object"
                )
            v.update(e)
        return v
    else:
